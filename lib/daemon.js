...
exports.run = async function () {
  ...
  if (nexusIsAvailable || onTriggerSSID) {
    log('switching to work profile')
    const didChange = await npmrc.enableWorkProfile()

    if (didChange) {
      await notify({
        title: 'npmrcd',
        message: 'Switched to "work" npmrc profile. Using private registry.'
      })
    }
    ...
    if (onTriggerSSID && !nexusIsAvailable) {
      await notify({
        title: 'npmrcd',
        message: `You are on the ${onTriggerSSID} network. The work registry is selected, but may not be accessible.`
      })
    }
  } else {
